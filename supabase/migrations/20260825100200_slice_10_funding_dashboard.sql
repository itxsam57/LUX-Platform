  ...
  from (
    select
      ...
      jsonb_build_object(
        ...
        'badge',case when badge_row.id is null then null else jsonb_build_object(
        ...
        'updatedAt',greatest(
          ...
          coalesce(transaction_row.updated_at,commitment.updated_at),
          coalesce(badge_row.updated_at,commitment.updated_at)
        )
      ) as payload
...
begin
  ...
  if trim(coalesce(requested_commitment_public_id,'')) !~ '^fnd[0-9a-f]{24}$'
     ...
     or normalized_key !~ '^[A-Za-z0-9:_-]{8,128}$' then
  ...
  if commitment_row.id is null then
    raise exception 'funding_commitment_not_found' using errcode = 'P0002';
  end if;
  ...
  join public.campaign_term_versions original_version on original_version.campaign_id=campaign.id
  where original_version.id=commitment_row.campaign_term_version_id
  ...
  where version_row.campaign_id=campaign_row.id
    and version_row.version=coalesce(campaign_row.current_terms_version,commitment_row.campaign_terms_version)
  limit 1;

  if source_version.id is null then
    raise exception 'funding_terms_not_found' using errcode = 'P0002';
  end if;
...
begin
  ...
  return jsonb_build_object(
    ...
    'capturedMinor',coalesce(transaction_row.captured_minor,0),
    'refundedMinor',coalesce(transaction_row.refunded_minor,0),
    'badge',case when badge_row.id is null then null else jsonb_build_object('key',badge_row.badge_key,'visibility',badge_row.visibility) end,
    ...
    'refundRequest',case when refund_row.id is null then null else jsonb_build_object(
      ...
      'amountMinor',refund_row.amount_minor,
      'reason',refund_row.reason
    ) end,
    'createdAt',commitment_row.created_at,
    'updatedAt',commitment_row.updated_at
  );
end;
$$;
...
begin
  ...
  if existing_row.id is not null then
    if existing_row.kind <> 'refund'
       or existing_row.amount_minor is distinct from requested_amount_minor
  ...
  if transaction_row.id is null
     or transaction_row.state not in ('authorized','captured','partially_refunded')
     or requested_amount_minor > transaction_row.authorized_minor-transaction_row.refunded_minor then
  ...
  insert into public.funding_change_requests(
    public_id,funding_commitment_id,kind,state,amount_minor,idempotency_key,reason
  ) values (
    candidate_public_id,commitment_row.id,'refund','pending',requested_amount_minor,normalized_key,normalized_reason
  ) returning * into existing_row;

  perform private.write_audit(
    auth.uid(),'funding_refund_requested','success','/app/funding/'||commitment_row.public_id,'fan',
    jsonb_build_object('commitmentPublicId',commitment_row.public_id,'requestPublicId',existing_row.public_id,'amountMinor',existing_row.amount_minor)
  );
