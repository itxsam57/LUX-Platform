...
begin
      ...
      jsonb_build_object(
        ...
        'materialChange', case
          when material.request_id is null then null
            ...
            'expectedDeliveryWindow', material.target_body ->> 'expectedDeliveryWindow'
          )
        end,
        'refundRequest', case
  ...
  join public.project_versions project_version
    on project_version.project_id = project.id
...
begin
  ...
  if char_length(normalized_window) not between 3 and 240
  ...
  if target_hash = original_version.terms_hash then
  ...
  if target_version.id is null then
    ...
    where campaign_id = original_version.campaign_id;
    ...
    insert into public.campaign_term_versions(
      ...
      original_version.campaign_id,
      next_version,
      original_version.project_revision,
      original_version.contract_term_version_id,
      normalized_body,
      target_hash,
      original_version.created_by_user_id
