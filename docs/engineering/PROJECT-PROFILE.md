...
The repository is currently at **Build Slice 0: repository and quality foundation**. It is not yet a production marketplace and must not be described or tested as though authentication, payments, uploads, consent, moderation, or creator workspaces already exist.
...
- `/` — public foundation status page.
- `/design-system` — internal foundation preview page.
- `/health` — public GET health contract.
...
- Public source, copy, styles, and foundation metadata: public.
...
- Tests must use deterministic synthetic, non-personal, non-explicit fixtures.

## Environment and secrets

Slice 0 requires no runtime secrets. Tracked `.env`, provider tokens, private keys, session data, generated screenshots/traces/videos, and private user content are forbidden. Future destructive tests must use isolated non-production providers and stores.
